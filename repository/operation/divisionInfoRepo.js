...
divisionInfoRepo.getShopNameById = async (id) => {
    const sql = `SELECT si.shop_name, di.division_name AS name FROM division_info di 
        JOIN project_info pi ON di.id = pi.division_id 
...
divisionInfoRepo.getShopNameByName = async (name) => {
    const sql = `SELECT si.shop_name, pi.project_name AS name FROM division_info di 
        JOIN project_info pi ON di.id = pi.division_id 
        JOIN shop_info si ON pi.id = si.project_id 
