...
goodsPayInfoRepo.getChildPaymentByShopNamesAndTime = async (shopNames, start, end) => {
    const sql = `SELECT IFNULL(SUM(A1.sale_amount), 0) AS sale_amount, 
            ...
            (CASE WHEN doa.onsale_date IS NULL OR 
				...
				WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) <= doa.onsale_date THEN 1
            ...
            if (params.search[i].value != 'old') {
                ...
                subsql = `${subsql} AND (EXISTS(
                    SELECT a2.brief_name FROM dianshang_operation_attribute a2 WHERE 
                        (a2.onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY) OR 
        ...
        if (row?.length) {
            ...
            if (goods_ids?.length) {
                ...
                sql = `SELECT d.goods_name, d.brief_name, d.operator, d.brief_product_line, 
                        ...
                        d.pit_target, (CASE WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) <= s.create_time 
                            THEN '新品30' 
                        WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) <= s.create_time 
                            THEN '新品60' 
                        WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY) <= s.create_time 
                            THEN '新品90' 
                    ...
                    SELECT goods_name, brief_name, operator, brief_product_line, 
                        ...
                        (CASE WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) <= onsale_date 
                            THEN '新品30' 
                        WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) <= onsale_date 
                            THEN '新品60' 
                        WHEN DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY) <= onsale_date 
                            THEN '新品90' 
                        ELSE '老品' END) AS onsale_info, o.dsr,d.sku_id,
...
goodsPayInfoRepo.getPromotionData = async (start, end, params, shopNames, linkIds,shopNames1) => {
    let page = parseInt(params.currentPage)
    let size = parseInt(params.pageSize)
    let offset = (page - 1) * size
    let p = [], result = {
    ...
    let operationSql=''
    if(params?.attribute!=''){
        operationSql=` AND d.link_attribute='${params?.attribute}'`
        if(params?.target!='')
        {
            operationSql+=` AND d.volume_target='${params?.target}'`
        }
    }
    let hitLineSql=''
    // 关联撞线表
    if(params.promotionType=='hit_line')
    {
        hitLineSql=`JOIN (
				SELECT a.shop_name,a.goods_id,a.cost_amount,b.base_amount,b.up_amount from goods_promotion_log a 
                JOIN goods_promotion_log b on a.goods_id=b.goods_id
                WHERE a.time_line='23:00:00' and b.base_amount>0
                AND a.start_time = '${end}'
                HAVING a.cost_amount+1>(b.base_amount+IFNULL(b.up_amount,0))
				) p ON s.goods_id=p.goods_id and s.shop_name=p.shop_name`
    }
    ...
    switch (params.promotionType) {
        ...
        case 'ip':
            ...
            break
        case 'unsalable_link':
            subsql = `${subsql} AND (d.userDef1 = '滞销' OR d.userDef6 = '滞销' OR d.link_attribute = '滞销')`
            break
    }
    if (shopNames != null) { 
        if (shopNames.length == 0) return result
    ...
    if (row?.length) {
        ...
        if (row?.length) {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                row[i].sku_sid = ''
                row[i].hasChild = hasChild
                // 代码行1647 row[i].sku_code ？不明白超超写的意思，查询的SQL中并没有sku_code这个select项
                row[i].id = row[i].goods_id || row[i].sku_code 
                row[i].parent_id = null
                if (row[i].goods_id) {
                    goods_ids = `${goods_ids}"${row[i].goods_id}",`
    ...
    const sql = `SELECT IFNULL(a1.sale_amount, 0) AS sale_amount, 
        ...
        LEFT JOIN(
            ...
            SELECT '' AS daily_promotion,'' AS daily_promotion_amount,'' AS scene_promotion,'' AS scene_promotion_amount,'' AS jd_express_promotion
                    ...
                    ,pay_time
            FROM tmall_promotion_info 
            WHERE pay_time BETWEEN ? AND ? AND period = 1 AND goods_id = ?
            GROUP BY goods_id,pay_time
        ) as a5 ON a1.date = a5.date 
        LEFT JOIN (      
 ...
 * 负利润链接统（排除无效链接、清滞销）、上架超过30天、链接近7天利润额小于0
 * @param {*} shopNames 
 * @param {*} start 
 * @param {*} end 
 * @param {*} shopNames1 
 * @returns 
 */
goodsPayInfoRepo.getNegativeProfitByShopNamesAndTime = async (shopNames, start, end, shopNames1) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildNegativeProfitByShopNamesAndTime = async (shopNames, start, end, shopNames1) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getNegativeProfitByLinksAndTime = async (links, start, end, links1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
        ...
        LEFT JOIN (SELECT IFNULL(SUM(profit), 0) AS profit, goods_id 
            FROM goods_pays WHERE goods_id NOT IN ('', '【无法匹配到商品】') 
                AND \`date\` BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND goods_id IN ("${links}") 
            GROUP BY goods_id) a ON a.goods_id = s.goods_id 
        LEFT JOIN dianshang_operation_attribute d ON d.goods_id = s.goods_id 
    ...
    if (links1) {
        sql = `SELECT SUM(count) AS count FROM (
            ...
            SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
                ...
                SELECT IFNULL(SUM(profit), 0) AS profit, goods_id 
                FROM goods_pays WHERE goods_id NOT IN ('', '【无法匹配到商品】') 
                    AND \`date\` BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND goods_id IN ("${links1}")
                GROUP BY goods_id) a ON a.goods_id = s.goods_id 
            WHERE s.onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 30 DAY) AND a.profit < 0) c`
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildNegativeProfitByLinksAndTime = async (links, start, end, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowProfitByShopNamesAndTime = async (shopNames, end ,shopNames1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND shop_name IN ("${shopNames}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowProfitByShopNamesAndTime = async (shopNames, end, shopNames1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowProfitByLinksAndTime = async (links, links1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND goods_id IN ("${links}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowProfitByLinksAndTime = async (links, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' AND shop_name IN ("${links}") GROUP BY goods_id) s 
        LEFT JOIN (SELECT IFNULL(SUM(profit), 0) AS profit, IFNULL(SUM(sale_amount), 0) AS sale_amount, goods_id 
            FROM goods_pays WHERE goods_id NOT IN ('', '【无法匹配到商品】') 
                AND \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) AND shop_name IN ("${links}") 
            GROUP BY goods_id) a ON a.goods_id = s.goods_id 
        LEFT JOIN dianshang_operation_attribute d ON d.goods_id = s.goods_id 
            AND ((d.userDef1 != '滞销' OR d.userDef1 IS NULL) AND (d.userDef7 = '滞销' OR d.userDef7 IS NULL) AND 
                (d.link_attribute = '滞销' OR d.link_attribute IS NULL)) AND d.is_price_comparison IS NULL 
    ...
    if (links1) {
        sql = `SELECT SUM(count) AS count, type, volume_target FROM (
            ...
            LEFT JOIN (SELECT IFNULL(SUM(profit), 0) AS profit, IFNULL(SUM(sale_amount), 0) AS sale_amount, goods_id 
                FROM goods_pays WHERE goods_id NOT IN ('', '【无法匹配到商品】') 
                    AND \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                    AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) AND shop_name IN ("${links1}")
                GROUP BY goods_id) a ON a.goods_id = s.goods_id 
            WHERE (s.onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) 
                OR (s.onsale_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) AND s.product_stage = '控')) 
                AND a.profit < a.sale_amount * 0.18 AND a.profit >= 0 AND s.volume_target IS NOT NULL 
 ...
 * @returns 
 */
goodsPayInfoRepo.getNullPromotionByShopNamesAndTime = async (shopNames, start, end, shopNames1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND shop_name IN ("${shopNames}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildNullPromotionByShopNamesAndTime = async (shopNames, start, end, shopNames1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getNullPromotionByLinksAndTime = async (links, start, end, links1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND goods_id IN ("${links}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildNullPromotionByLinksAndTime = async (links, start, end, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPromotionByShopNamesAndTime = async (shopNames, start, end, promotion_rate, shopNames1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND shop_name IN ("${shopNames}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPromotionByShopNamesAndTime = async (shopNames, start, end, promotion_rate, shopNames1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPromotionByLinksAndTime = async (links, start, end, promotion_rate, links1) => {
    let sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND goods_id IN ("${links}") GROUP BY goods_id) s 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPromotionByLinksAndTime = async (links, start, end, promotion_rate, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowROIByShopNamesAndTime = async (shopNames,end) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowROIByShopNamesAndTime = async (shopNames,end) => {
 ...
 * @returns
 */
goodsPayInfoRepo.getLowROIByShopNamesAndTime1 = async (shopNames,end) => {
    const sql = `SELECT COUNT(DISTINCT a.goods_id) AS count FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, goods_id 
 ...
 * @returns
 */
goodsPayInfoRepo.getChildLowROIByShopNamesAndTime1 = async (shopNames,end) => {
    const sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowROIByShopNamesAndTime2 = async (shopNames, end ,shopNames1) => {
 ...
 * @returns 
 */ 
goodsPayInfoRepo.getChildLowROIByShopNamesAndTime2 = async (shopNames,end, shopNames1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowROIByLinksAndTime = async (links) => {
    const sql = `SELECT COUNT(DISTINCT a.goods_id) AS count FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, goods_id 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowROIByLinksAndTime = async (links) => {
    const sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowROIByLinksAndTime1 = async (links) => {
    const sql = `SELECT COUNT(DISTINCT a.goods_id) AS count FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, goods_id 
 ...
 * @returns
 */
goodsPayInfoRepo.getChildLowROIByLinksAndTime1 = async (links) => {
    const sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowROIByLinksAndTime2 = async (links, links1) => {
    let sql = `SELECT COUNT(DISTINCT a.goods_id) AS count FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, goods_id 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowROIByLinksAndTime2 = async (links, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPlanROIByShopNamesAndTime = async (shopNames, days1, days2) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPlanROIByShopNamesAndTime = async (shopNames, days1, days2) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPlanROIByShopNamesAndTime1 = async (shopNames, days1, days2) => {
    const sql = `SELECT SUM(count) AS count FROM (
        SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
                IFNULL(SUM(trans_amount), 0) AS trans_amount, goods_id FROM goods_promotion_plan 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPlanROIByShopNamesAndTime1 = async (shopNames, days1, days2) => {
    const sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPlanROIByShopNamesAndTime2 = async (shopNames, days1, days2, shopNames1) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPlanROIByShopNamesAndTime2 = async (shopNames, days1, days2, shopNames1) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPlanROIByLinksAndTime = async (links, days1, days2) => {
    const sql = `SELECT SUM(count) AS count FROM (
        SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
                IFNULL(SUM(trans_amount), 0) AS trans_amount, goods_id FROM goods_promotion_plan 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPlanROIByLinksAndTime = async (links, days1, days2) => {
    const sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getLowPlanROIByLinksAndTime2 = async (links, days1, days2, links1) => {
    let sql = `SELECT SUM(count) AS count FROM (
        SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
                IFNULL(SUM(trans_amount), 0) AS trans_amount, goods_id FROM goods_promotion_plan 
        ...
        sql = `${sql}
            ...
            SELECT COUNT(DISTINCT s.goods_id) AS count FROM (
                ...
                WHERE platform = '自营' AND (userDef1 != '滞销' OR userDef1 IS NULL) 
                    ...
                    AND onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)) s JOIN (
                SELECT IFNULL(SUM(real_sale_amount), 0) AS real_sale_amount, 
                    IFNULL(SUM(profit), 0) AS profit, IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                    goods_id, sku_id FROM goods_sale_info WHERE goods_id NOT IN ('', '【无法匹配到商品】') 
                    AND \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL ${days2} DAY) 
                    AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) AND goods_id IN ("${links1}") 
                GROUP BY goods_id, sku_id) a ON a.goods_id = s.goods_id AND a.sku_id = s.sku_id 
                LEFT JOIN (SELECT IFNULL(SUM(trans_amount), 0) AS trans_amount, promotion_name, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getChildLowPlanROIByLinksAndTime2 = async (links, days1, days2, links1) => {
    let sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
 ...
 * @returns 
 */
goodsPayInfoRepo.getInvalidByShopNamesAndTime = async (shopNames, start, end, shopNames1) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getInvalidByLinksAndTime = async (links, start, end, links1) => {
    let sql = `SELECT COUNT(DISTINCT s1.goods_id) AS count, IFNULL(SUM(s1.sale_amount), 0) AS sale_amount, 
            IFNULL(SUM(s1.profit), 0) AS profit FROM (
            SELECT s.goods_id, MIN(s.create_time) AS create_time, a.sale_amount, a.profit 
            FROM (SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsPayInfoRepo.getImportByLinks2 = async (links) => {
    const sql = `SELECT COUNT(DISTINCT s.goods_id) AS count FROM dianshang_operation_attribute d 
        JOIN (SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
            WHERE create_time IS NOT NULL AND is_shelf = '是' 
                AND goods_id IN ("${links}") GROUP BY goods_id) s ON d.goods_id = s.goods_id 
 ...
 * @returns 
 */
goodsPayInfoRepo.getUnsalableCodeByShopNames = async (shopNames, start, end, shopNames1) => {
    let sql = `SELECT IFNULL(SUM(cost_amount), 0) AS cost_amount, IFNULL(SUM(sale_amount), 0) AS sale_amount, 
        ...
        JOIN (SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, 
            ...
            FROM goods_pay_info WHERE date BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND shop_name IN ("${shopNames}") 
    ...
    if (shopNames1) {
        sql = `SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, SUM(profit) AS profit 
            FROM (${sql}
                ...
                GROUP BY spcode, s.sku_id, s.goods_id,rate) a 
                JOIN danpin.goods_info g ON g.\`商品编码\` = a.spcode AND g.\`备注标签\` IN ('滞销', '销完下架')
                JOIN (SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, 
                        SUM(profit) AS profit, goods_id, sku_id 
                    FROM goods_pay_info WHERE date BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND shop_name IN ("${shopNames1}") 
    ...
    let sql1 = `SELECT IFNULL(SUM(amount), 0) AS amount FROM (
        ...
        JOIN (SELECT IFNULL(c.\`商品编码\`, s.sys_sku_id) AS sku_id FROM (
 ...
 * @returns 
 */
goodsPayInfoRepo.getUnsalableCodeByLinks = async (links, start, end, links1) => {
    let sql = `SELECT IFNULL(SUM(cost_amount), 0) AS cost_amount, IFNULL(SUM(sale_amount), 0) AS sale_amount, 
            IFNULL(SUM(profit), 0) AS profit 
        FROM (SELECT IFNULL(c.\`商品编码\`, s.sys_sku_id) AS sku_id, s.goods_id FROM (
            SELECT sys_sku_id, goods_id FROM jst_goods_sku 
        ...
        JOIN (SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, 
                SUM(profit) AS profit, goods_id, sku_code 
            FROM goods_pay_info WHERE date BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND shop_name IN ("${links}") 
            GROUP BY goods_id, sku_code) b ON a.goods_id = b.goods_id AND a.sku_id = b.sku_code`
    let params = [start, end]
    if (links1) {
        sql = `SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, SUM(profit) AS profit 
            FROM (${sql}
                ...
                JOIN (SELECT SUM(cost_amount) AS cost_amount, SUM(sale_amount) AS sale_amount, 
                        SUM(profit) AS profit, goods_id, sku_id 
                    FROM goods_pay_info WHERE date BETWEEN DATE_SUB(?, INTERVAL 1 DAY) AND DATE_SUB(?, INTERVAL 1 DAY) AND goods_id IN ("${links1}") 
                    GROUP BY goods_id, sku_id) b ON a.goods_id = b.goods_id AND a.sku_id = b.sku_id
                GROUP BY a.goods_id, a.sku_id, a.rate) k`
        params.push(start, end)
    }
 ...
 * @returns 
 */
goodsPayInfoRepo.getIpByShopNames = async (shopNames, months, start, end) => {
 ...
 * @returns 
 */
goodsPayInfoRepo.getIpByLinks = async (links, months, start, end, links1) => {
    let presql = `SELECT COUNT(a.sale_amount < t.amount) AS count, IFNULL(SUM(a.sale_amount), 0) AS sale_amount, 
            IFNULL(SUM(a.profit), 0) AS profit, IFNULL(SUM(t.amount), 0) AS amount FROM (
        SELECT IF(d.platform = '自营', d.brief_name, d.goods_id) AS goodsid 
            FROM dianshang_operation_attribute d 
 ...
 * @returns 
 */
goodsPayInfoRepo.getUnsalableByLinks = async (links, links1) => {
...
goodsPayInfoRepo.getProductStage = async (shopNames, userNames, month1, month2, percent1, percent2, is_new) => {
    const presql = shopNames?.length ? `AND shop_name IN (${shopNames})` : `AND operator IN (${userNames})`
    const subsql = is_new ? 'IFNULL(s.create_time, d.onsale_date) >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)' : 
        'IFNULL(s.create_time, d.onsale_date) < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)'
    const sql = `SELECT product_stage, COUNT(1) AS count, SUM(IF(volume_target = '超大', 1, 0)) AS super, 
            SUM(IF(volume_target = '大', 1, 0)) AS big, SUM(IF(volume_target = '中', 1, 0)) AS mediumn, 
            SUM(IF(volume_target = '小', 1, 0)) AS small, SUM(IF(new_tag = 1, 1, 0)) AS tag_num, 
            SUM(IF(new_tag = 0, 1, 0)) AS ntag_num, SUM(IF(ta.status = '冷起', 1, 0)) AS astatus1, 
            SUM(IF(ta.status = '爆品', 1, 0)) AS astatus2, SUM(IF(ta.status = '冲向', 1, 0)) AS astatus3, 
            SUM(IF(link_stage = 'H1', 1, 0)) AS stage1, SUM(IF(link_stage = 'H2', 1, 0)) AS stage2, 
            SUM(IF(link_stage = 'H3', 1, 0)) AS stage3, SUM(IF(link_stage = 'H4', 1, 0)) AS stage4, 
            SUM(IF(link_stage = 'H5', 1, 0)) AS stage5, SUM(IF(a.profit < 0 
                AND IFNULL(s.create_time, d.onsale_date) <= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY), 1, 0)) 
                AS negative_profit, SUM(IF(a.profit >= 0 AND a.profit < a.sale_amount * 0.18 
                AND IFNULL(s.create_time, d.onsale_date) <= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY), 1, 0)) 
                AS low_profit, SUM(IF(a.promotion_amount = 0, 1, 0)) AS none_promotion, 
            SUM(IF(d.play = '微付费', 1, 0)) AS play1, SUM(IF(d.play = '中付费', 1, 0)) AS play2,
            SUM(IF(d.play = '高付费', 1, 0)) AS play3, SUM(IF(da.goods_id IS NULL, 0, 1)) AS anum,
            SUM(IF(mt.goods_id IS NOT NULL AND mt.amount <= a.sale_amount 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY), 1, 0)) AS goal_achieve,
            SUM(IF(mt.goods_id IS NOT NULL AND mt.amount > a.sale_amount 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY), 1, 0)) AS goal_not_achieve
        FROM (SELECT IF(platform = '自营', brief_name, goods_id) AS goodsid, volume_target, 
                IF(is_circulation = '是', '流转', product_stage) AS product_stage, 
                new_tag, link_stage, play, onsale_date, shop_name FROM dianshang_operation_attribute 
            WHERE IF(platform = '自营', brief_name IS NOT NULL, goods_id IS NOT NULL)
                AND IF(platform = '天猫部', (link_state != '下架' OR link_state IS NULL), 
                    IF(platform = '自营', (userDef1 != '下柜' OR userDef1 IS NULL), 1=1)) 
                ${presql}
            GROUP BY goodsid, shop_name,
                volume_target, new_tag, link_stage, play, onsale_date, 
                IF(is_circulation = '是', '流转', product_stage)) d 
        LEFT JOIN (SELECT goods_id, MIN(create_time) AS create_time 
            FROM jst_goods_sku WHERE is_shelf = '是' GROUP BY goods_id) s ON s.goods_id = d.goodsid
        LEFT JOIN tmall_new_activity ta ON ta.goods_id = d.goodsid 
            AND ta.date = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) 
        LEFT JOIN (SELECT goods_id, IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(profit), 0) AS profit, IFNULL(SUM(promotion_amount), 0) AS promotion_amount 
            FROM goods_pays_stats WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY goods_id) a ON a.goods_id = d.goodsid 
        LEFT JOIN (SELECT goods_id FROM danpin.activity WHERE \`date\` = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
            GROUP BY goods_id) da ON da.goods_id = d.goodsid
        LEFT JOIN (SELECT goods_id, FORMAT(SUM(amount), 2) AS amount FROM(
                ...
                WHERE month = "${month1}"
                UNION ALL 
                SELECT goods_id, amount * ${percent2} AS amount FROM goods_monthly_sales_target 
                WHERE month = "${month2}") m GROUP BY goods_id) mt ON mt.goods_id = d.goodsid
        WHERE ${subsql} GROUP BY product_stage`
    const result = await query(sql)
    return result
}

goodsPayInfoRepo.getGoodsLowROI = async (shopNames, userNames, is_new) => {
    let presql = shopNames?.length ? `AND shop_name IN (${shopNames})` : `AND operator IN (${userNames})`
    let subsql = is_new ? `AND s.create_time >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) 
        AND IF(d.platform = '天猫部', a.sale_amount < a.promotion_amount + a.cost_amount, 
            a.promotion_amount * a.cost_amount < a.sale_amount * 
            (2.67 * a.promotion_amount + 2 * a.cost_amount - 1.34 * a.sale_amount))` :
        `AND s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) 
        AND a.promotion_amount * a.cost_amount < a.sale_amount * 
            (2.67 * a.promotion_amount + 2 * a.cost_amount - 1.34 * a.sale_amount)`
    let subsql1 = is_new ? `AND d.onsale_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)` :
        `AND d.onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)`
    const sql = `SELECT COUNT(1) AS count, product_stage FROM (
        SELECT d.product_stage, d.goods_id FROM (
            SELECT goods_id, platform, IF(is_circulation = '是', '流转', product_stage) AS product_stage 
            FROM dianshang_operation_attribute WHERE goods_id IS NOT NULL
                AND IF(platform = '天猫部', (link_state != '下架' OR link_state IS NULL), 1=1) 
                ${presql}
            GROUP BY goods_id, platform, IF(is_circulation = '是', '流转', product_stage)) d 
        LEFT JOIN (SELECT goods_id, MIN(create_time) AS create_time 
            FROM jst_goods_sku WHERE is_shelf = '是' GROUP BY goods_id) s ON s.goods_id = d.goods_id 
        LEFT JOIN (SELECT IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, 
                IFNULL(SUM(promotion_amount), 0) AS promotion_amount, goods_id
            FROM goods_pays_stats WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY goods_id) a ON a.goods_id = d.goods_id
        WHERE s.create_time <= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
            ${subsql}
        GROUP BY d.product_stage, d.goods_id
        UNION ALL 
        SELECT d.product_stage, d.brief_name AS goods_id FROM (
            SELECT brief_name, onsale_date, IF(is_circulation = '是', '流转', product_stage) AS product_stage 
            FROM dianshang_operation_attribute WHERE (userDef1 != '下柜' OR userDef1 IS NULL) 
                AND brief_name IS NOT NULL AND platform = '自营' 
            GROUP BY brief_name, onsale_date, IF(is_circulation = '是', '流转', product_stage)) d 
        LEFT JOIN (SELECT IFNULL(SUM(real_sale_amount), 0) AS real_sale_amount, 
                IFNULL(SUM(promotion_amount), 0) AS promotion_amount, IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(profit), 0) AS profit, goods_id
            FROM goods_sales WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY goods_id) a ON a.goods_id = d.brief_name
        WHERE d.onsale_date <= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
            ${subsql1}
            AND a.sale_amount * (2 * a.profit - 0.3) < a.promotion_amount * (a.real_sale_amount + a.profit - 0.15)
        GROUP BY d.product_stage, d.brief_name) a GROUP BY product_stage`
    const result = await query(sql)
    return result || []
}

goodsPayInfoRepo.getGoodsLowROIPlan = async (shopNames, userNames, is_new) => {
    let presql = shopNames?.length ? `AND shop_name IN (${shopNames})` : `AND operator IN (${userNames})`
    let subsql = is_new ? `AND s.create_time >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) 
        AND IF(d.platform = '天猫部', p.trans_amount * a.sale_amount < a.cost_amount * (p.trans_amount + p.pay_amount), 
            1.34 * p.trans_amount * a.sale_amount + p.pay_amount * a.cost_amount < 
            2 * a.sale_amount * p.pay_amount + 0.67 * p.pay_amount * a.cost_amount + 2 * p.pay_amount * a.cost_amount)` :
        `AND s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY) 
        AND 1.34 * p.trans_amount * a.sale_amount + p.pay_amount * a.cost_amount < 
            2 * a.sale_amount * p.pay_amount + 0.67 * p.pay_amount * a.cost_amount + 2 * p.pay_amount * a.cost_amount`
    let subsql1 = is_new ? `AND d.onsale_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)` :
        `AND d.onsale_date < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY)`
    const sql = `SELECT COUNT(1) AS count, product_stage FROM (
        ...
        LEFT JOIN (SELECT goods_id, MIN(create_time) AS create_time 
            FROM jst_goods_sku WHERE is_shelf = '是' GROUP BY goods_id) s ON s.goods_id = d.goods_id 
        LEFT JOIN (SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name,
                IFNULL(SUM(trans_amount), 0) AS trans_amount, goods_id 
            FROM goods_promotion_plan WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY goods_id, promotion_name) p 
            ON p.goods_id = d.goods_id 
        LEFT JOIN (SELECT IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                IFNULL(SUM(cost_amount), 0) AS cost_amount, goods_id
            FROM goods_pays_stats WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) GROUP BY goods_id) a ON a.goods_id = d.goods_id
        ...
        LEFT JOIN (SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name,
            ...
            WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
                AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) 
            GROUP BY goods_id, promotion_name) p ON p.goods_id = d.brief_name
        LEFT JOIN (SELECT IFNULL(SUM(real_sale_amount), 0) AS real_sale_amount, 
                IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
                IFNULL(SUM(profit), 0) AS profit, goods_id
            FROM goods_sales WHERE \`date\` BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
        ...
        WHERE d.onsale_date <= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
            ${subsql1}
            AND IF(p.promotion_name = '全站营销', 
                p.trans_amount * (2 * a.promotion_amount + 2 * a.profit - 0.3 * a.real_sale_amount) < 
                p.pay_amount * (1.85 * a.real_sale_amount + a.promotion_amount + a.profit), 
                p.trans_amount * (a.promotion_amount +  a.profit) < 
                p.pay_amount * (a.real_sale_amount + 0.5 * a.profit + 0.5 * promotion_amount))
        GROUP BY d.product_stage, d.brief_name) a GROUP BY product_stage`
    const result = await query(sql)
    return result || []
}
...
// 获取撞线推品统计
goodsPayInfoRepo.getHitLineTotal = async (shopNames,end)=>{
