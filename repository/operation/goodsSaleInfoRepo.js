...
goodsSaleInfoRepo.getPromotionData = async (start, end, params, shopNames, linkIds,shopNames1) => {
    ...
    if (row?.length && row[0].count) {
        ...
        sql = `SELECT s.goods_id, s.shop_name, si.shop_id, a1.pay_amount, a1.brushing_amount, 
            ...
            FORMAT(IF(a1.words_market_vol > 0, a1.words_vol / a1.words_market_vol * 100, 0), 2) 
            AS market_rate, FORMAT(IF(a1.order_num > 0, a1.refund_num / a1.order_num * 100, 0), 2) 
            AS refund_rate, a1.profit, FORMAT(IF(a1.sale_amount > 0, 
                a1.profit / a1.sale_amount * 100, 0), 2) AS profit_rate, 
            ...
            FROM (SELECT goods_id, MIN(create_time) AS onsale_date, shop_name FROM jst_goods_sku 
                ...
                FROM dianshang_operation_attribute WHERE platform = '自营' 
                    ...
                    IFNULL(SUM(profit), 0) AS profit, goods_id, shop_name 
                FROM goods_sales_stats WHERE \`date\` BETWEEN ? AND ? 
                    AND goods_id NOT IN ('', '【无法匹配到商品】') GROUP BY goods_id, shop_name) a1 
                ON a1.goods_id = s.goods_id AND a1.shop_name = s.shop_name 
            LEFT JOIN (SELECT IFNULL(SUM(sale_amount), 0) AS sale_amount, 
                    IFNULL(SUM(promotion_amount), 0) AS promotion_amount, goods_id, shop_name 
                FROM goods_sales_stats WHERE \`date\` BETWEEN "${preStart}" AND "${preEnd}" 
...
goodsSaleInfoRepo.getDataDetailTotalByTime = async(goods_id, start, end) => {
    const sql = `SELECT IFNULL(a1.sale_amount, 0) AS sale_amount, 
			...
			FORMAT(IF(a1.shop_name='京东自营-厨具',IFNULL(a1.sale_amount, 0)*0.07, 
                IFNULL(a1.sale_amount, 0)*0.07 +IFNULL(a1.pay_amount, 0)*0.02),2) AS tax,
 ...
 * @returns 
 */
goodsSaleInfoRepo.getChildLowProfitByLinksAndTime = async (links, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT MIN(create_time) AS create_time, goods_id FROM jst_goods_sku
 ...
 * @returns 
 */
goodsSaleInfoRepo.getChildLowROIByLinksAndTime2 = async (links, links1) => {
    let sql = `SELECT COUNT(1) AS count, type, volume_target FROM (
        SELECT IF(s.create_time < DATE_SUB(CURRENT_DATE(), INTERVAL 60 DAY), '老品', '新品') AS type, 
            s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(promotion_amount), 0) AS promotion_amount, 
 ...
 * @returns 
 */
goodsSaleInfoRepo.getChildLowPlanROIByLinksAndTime = async (links, days1, days2) => {
    const sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
 ...
 * @returns 
 */
goodsSaleInfoRepo.getChildLowPlanROIByLinksAndTime1 = async (links, days1, days2) => {
    const sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
 ...
 * @returns 
 */
goodsSaleInfoRepo.getChildLowPlanROIByLinksAndTime2 = async (links, days1, days2, links1) => {
    let sql = `SELECT SUM(count) AS count, type, volume_target FROM (
        SELECT COUNT(1) AS count, type, volume_target FROM (
            SELECT '老品' AS type, s.goods_id, d.volume_target FROM (
            SELECT IFNULL(SUM(pay_amount), 0) AS pay_amount, promotion_name, 
...
goodsSaleInfoRepo.getskuCodeInfo = async(goods_id, start, end) => {
